...
\newcommand*{\titleBC}{ % Command for including the title page in the document
	\begingroup
	...
	\pagestyle{empty}
	27 February 2015 % Year published
...
\begin{abstract}
We have created a molecular dynamics simulation to investigate the behavior of a homegenous system of e.g. argon particles. In our simulation, the particles interact with a Lennard-Jones potential. The velocity Verlet algorithm is used to integrate the equations of motion and update the postition and velocity of the particles. The initial conditions consist of the particles spaced out evenly in a FCC lattice, with an initial velocity drawn from the Maxwell-Boltzman distribution as a function of the intial temperature. We use periodic boundary conditions to only simulate the interaction between argon atoms.
...
Using statistical physics, a lot of properties of a system can be derived by looking at the behaviour of many particles. Because direct experimentation is impossible, it is an ideal subject to simulate in a computer program. Starting out with the right inter-particle potential, we can simulate the interaction and movement of the particles. Combined with the righ initial and boundary conditions, it is possible to determine some thermodynamical properties of a material like the temperature, spacial correlation, pressure and diffusion.
...
Also the initial placement of the particles were visualised to assure that the particles were placed correctly in the FCC latice which is shown in figure \ref{fig:init_place}. This visual aid was also used to spot any discreptancies in the movement of the particles for example at periodic boundaries.

\begin{figure}[H]
\centering
\includegraphics[scale=0.5]{fcc.png}
\caption{Initial placement of particles in FCC latice }
